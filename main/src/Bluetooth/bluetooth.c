...
static const uint16_t GATTS_CHAR_UUID_TEST_TEMP = 0xFF04;//
...
static const uint8_t temp_value[20] = {0x00};

/// @brief GATT服务表，添加了一个特征值
...
volatile bool notify_flag = false; //！是否打开notify通知
...
static void get_temp(void *arg)
{
    while (1)
    {
        if (notify_flag == true)
        {
            ...
            if(temp > 28)//!此处可根据mpu6050计算出的姿态值
            {
               esp_ble_gatts_set_attr_value(heart_rate_handle_table[IDX_CHAR_VAL_TEMP], sizeof(warning), warning);
               esp_ble_gatts_send_indicate(heart_rate_profile_tab[0].gatts_if, heart_rate_profile_tab[0].conn_id,heart_rate_handle_table[IDX_CHAR_VAL_TEMP], sizeof(warning), warning, false);//发送数据
            }
            else
            {
                esp_ble_gatts_set_attr_value(heart_rate_handle_table[IDX_CHAR_VAL_TEMP], sizeof(normal), normal); 
                esp_ble_gatts_send_indicate(heart_rate_profile_tab[0].gatts_if, heart_rate_profile_tab[0].conn_id,heart_rate_handle_table[IDX_CHAR_VAL_TEMP], sizeof(normal), normal, false);//发送数据
            }
        }else{
            vTaskDelete(NULL);
        }
